...
from fonctions import chooseanat
from fonctions import _1_fMRI_preTTT_in_fMRIspace
from fonctions import _2_coregistration_to_norm
from fonctions import _3_mask_fMRI
from fonctions import _4_check_mask
from fonctions import _5_anat_to_fMRI
from fonctions import _6_Melodic
from fonctions import _7_post_TTT
from fonctions import _8_fMRI_to_anat
from fonctions import _9_coregistration_to_template_space
from fonctions import _10_Correl_matrix
from fonctions import _11_Seed_base_many_regionsatlas
from fonctions import _12_fMRI_QC
from fonctions import _14_fMRI_QC_matrix
from fonctions import _100_Data_Clean
from fonctions import _200_Data_QC
...
def preprocess_data(species,all_ID, all_Session, all_data_path, all_Session_max, stdy_template, stdy_template_mask,
    ...
    for ID, Session, data_path, max_ses in zip(all_ID, all_Session, all_data_path, all_Session_max):
        ...
        (path_rawfunc,path_func, dir_rawfmap,dir_fmap, dir_rs_prepro, dir_prepro_orig, dir_prepro_acpc, dir_prepro_template,
         dir_residuals, dir_residuals_acpc, dir_residuals_template) = getpath.func(data_path,reference)

        if not ope(path_func):
            os.makedirs(path_func)
        if not ope(dir_rs_prepro):
            os.makedirs(dir_rs_prepro)
        if not ope(dir_prepro_orig):
            os.makedirs(dir_prepro_orig)
        ...
        list_RS = sorted(glob.glob(opj(path_rawfunc, endfmri)))
        ...
        if len(list_RS) == 0:
            nl = ('ERROR : No func image found, we are look for an image define such as opj(dir_fMRI_Refth_RS, endfmri) and here it is ' +
                  str(opj(path_rawfunc, endfmri)) + ' I would check how you define "endfmri"')
        ...
        for imageF in list_RS:
            ...
            if ope(opj(path_rawfunc, extract_filename(imageF) + '_confounds.tsv')):
                confounds_df = pd.read_csv(opj(path_rawfunc, extract_filename(imageF) + '_confounds.tsv'), sep='\t')
        ...
        list_map = sorted(glob.glob(opj(dir_rawfmap, endmap)))
        nl = "looking for fmap image with the command glob.glob(" + str(opj(dir_rawfmap, endmap))
        run_cmd.msg(nl, diary_file, 'OKGREEN')
        ...
        if len(list_RS)>0:
            ...
            if len(list_map) == 0:
                ...
                comp = check_nii.comphd(opj(dir_rawfmap, RS_map[0]), opj(path_rawfunc, RS[int(REF_int)-1]))
            ...
            list_json = sorted(glob.glob(opj(path_rawfunc, endjson)))
            ...
            if 1 in Skip_step:
               ...
               _1_fMRI_preTTT_in_fMRIspace.preprocess_data(dir_prepro_orig,
                                                                     RS,
                                                                     list_RS,
                                                                     nb_run,
                                                                     T1_eq,
            ...
            if 3 in Skip_step:
                ...
                _3_mask_fMRI.Refimg_to_meanfMRI(anat_func_same_space, BASE_SS_coregistr, TfMRI, dir_prepro_orig, dir_prepro_acpc,  use_master_for_Allineate,
                                                          dir_prepro_template, RS, nb_run, REF_int, ID, dir_prepro, brainmask, V_mask, W_mask, G_mask, dilate_mask,
            ...
            if 7 in Skip_step:
                ...
                _7_post_TTT.signal_regression(dir_prepro_orig, dir_residuals_acpc,
                                                        ...
                                                        do_not_correct_signal, band, extract_Vc, extract_GS, overwrite, sing_afni, diary_file)
            ...
            if 9 in Skip_step:
                ...
                _9_coregistration_to_template_space.to_common_template_space(deoblique, dir_prepro_orig, dir_prepro_acpc, dir_prepro_template,
            ...
            if 11 in Skip_step:
                ...
                _11_Seed_base_many_regionsatlas.SBA(SBAspace, BASE_SS_coregistr, erod_seed, dir_prepro_orig, dir_prepro_acpc,
                                                              dir_prepro_template, RS, nb_run, selected_atlases, panda_files, oversample_map,
            ...
            if 12 in Skip_step:
                ...
                _12_fMRI_QC.fMRI_QC(correction_direction, dir_prepro_orig, dir_prepro_template, RS, nb_run, sing_afni, diary_file)
