...
def to_common_template_space(deoblique, dir_fMRI_Refth_RS_prepro1, dir_fMRI_Refth_RS_prepro2, dir_fMRI_Refth_RS_prepro3,
    ...
    if opj(dir_fMRI_Refth_RS_prepro3,'tmp') == False:
    ...
    for input2, output2, output3 in zip([opj(dir_fMRI_Refth_RS_prepro1, 'Mean_Image.nii.gz'), opj(dir_fMRI_Refth_RS_prepro1, 'Ref_anat_in_fMRI.nii.gz')],
        ...
        print(bcolors.OKGREEN + 'orientation orig of the func image is ' + orientation_orig + bcolors.ENDC)
        ...
        if deoblique == 'header':
            ...
            raise Exception(bcolors.FAIL + 'ERROR: wrong deoblique name' + bcolors.ENDC)
        ...
        print(bcolors.OKGREEN + "starting func to template space on mean image and anat test transormation" + bcolors.ENDC)
    ...
    for i in range(0, int(nb_run)):
        ...
        root_RS = extract_filename(RS[i])
        if ope(opj(dir_fMRI_Refth_RS_prepro3, 'tmp')):
        ...
        if ope(input2) == False:
            ...
            if ope(input2) == False:
                    raise Exception(bcolors.FAIL + 'ERROR: corrected func image in func space not found' + bcolors.ENDC)
