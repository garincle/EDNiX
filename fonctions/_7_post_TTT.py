...
def signal_regression(dir_fMRI_Refth_RS_prepro1, dir_RS_ICA_native,
    ...
    for i in range(int(nb_run)):
        ...
        command = (sing_afni + '3dcalc' + overwrite + ' -a ' + input +
                   ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS.nii.gz') +
                   ' -expr "a"')
    ...
    for i in range(int(nb_run)):
        ...
        for extract_type, img_name ,suffix in zip([extract_exterior_CSF, extract_WM, extract_GS],
            ...
            if extract_type == True:
                ...
                Bp1d = open(opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS' + suffix + '.1D'), 'w')
        ...
        if extract_Vc == True:
                msk = ants.image_read(opj(dir_fMRI_Refth_RS_prepro1,'Vmask.nii.gz'))
                countVmask = int(msk.sum())

                if countVmask>10:
                    command = (sing_afni + '3dmaskSVD' + overwrite + ' -polort 2 -vnorm -mask ' + opj(
                        dir_fMRI_Refth_RS_prepro1, 'Vmask.nii.gz') +
                               ' ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS.nii.gz'))
                    val, _ = run_cmd.get(command, diary_file)
                    list1D = val.decode("utf-8").split('\n')
                    Bp1d = open(opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS_Vc.1D'), 'w')
                    for i in range(len(list1D)):
                        line = ' '.join(list1D[i].split()) + '\n'
                        Bp1d.write(line)
                    Bp1d.close()
        ...
        hd = ants.image_header_info(opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS.nii.gz'))
        ...
        Bp1d = open(opj(dir_fMRI_Refth_RS_prepro1, root_RS + 'bandpass_rall.1D'), 'w')
        for i in range(len(list1D)):
            line = ' '.join(list1D[i].split()) + '\n'
            Bp1d.write(line)
        Bp1d.close()
    ...
    if do_not_correct_signal == False:
        for i in range(int(nb_run)):
            try:
                ...
                command = (sing_afni + '3dDeconvolve -input ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS.nii.gz') +
                           ' -mask ' + opj(dir_fMRI_Refth_RS_prepro1, 'maskDilat.nii.gz') +
                           ' -ortvec ' + opj(dir_fMRI_Refth_RS_prepro1,
                                             root_RS + 'bandpass_rall.1D') + ' bandpass_rall' +
                ...
                zscore_command = (sing_afni + '3dTstat -mean -overwrite' +
                                  ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,'mean.nii.gz') +
                                  ...
                                  ' -expr "(a-b)/c" -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,
                                                                    root_RS + '_residual.nii.gz'))

                psc_command = (sing_afni + '3dTstat -mean -overwrite -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,
                                                                                     'mean_func.nii.gz') +
                               ' ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_residual.nii.gz') +
                               ' && 3dcalc -overwrite -a ' + opj(dir_fMRI_Refth_RS_prepro1,
                                                                 root_RS + '_residual.nii.gz') +
                               ' -b ' + opj(dir_fMRI_Refth_RS_prepro1, 'mean_func.nii.gz') +
                               ' -expr "((a - b) / b) * 100" -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,
                                                                             root_RS + '_residual.nii.gz'))
                ...
                command = (sing_afni + '3dcalc' + overwrite +
                           ...
                           ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_3dDeconvolve_failed.nii.gz') +
                           ' -expr "a"')
                run_cmd.do(command, diary_file)
            ...
            if not ope(opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_residual.nii.gz')):
                ...
                command = (sing_afni + '3dcalc' + overwrite +
                           ' -a ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfwS.nii.gz') +
                           ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,
                                             root_RS + '_3dDeconvolve_failed.nii.gz') + ' -expr "a"')
                run_cmd.do(command, diary_file)
