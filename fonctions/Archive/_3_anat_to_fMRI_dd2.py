...
def Refimg_to_meanfMRI(anat_func_same_space, volumes_dir, BASE_SS_coregistr, TfMRI, correction_direction, dir_fMRI_Refth_RS_prepro1, dir_fMRI_Refth_RS_prepro2, 
    ...
    if anat_func_same_space == True:
        ...
        command = '3dZeropad -I 200 -S 200 -A 200 -P 200 -L 200 -R 200 -S 200 -prefix ' + opj(dir_fMRI_Refth_RS_prepro2,'orig_anat_for_fMRI.nii.gz') + ' ' + opj(dir_prepro, ID + '_acpc_cropped' + TfMRI + '.nii.gz') + ' -overwrite'
        spco([command], shell=True)
        ...
        command = '3dZeropad -I 200 -S 200 -A 200 -P 200 -L 200 -R 200 -S 200 -prefix ' + opj(dir_fMRI_Refth_RS_prepro2,'Gmask.nii.gz') + ' ' + G_mask + ' -overwrite'
        spco([command], shell=True)
        command = '3dAllineate' + overwrite + ' -final NN -overwrite -1Dmatrix_apply ' + mvt_shft + \
    ...
    mvt_shft_INV_ANTs
    spco([command], shell=True)
    command = '3dcalc' + overwrite + ' -a ' + opj(dir_fMRI_Refth_RS_prepro1, 'maskDilat.nii.gz') + ' -b ' + opj(dir_fMRI_Refth_RS_prepro1, 'Ref_anat_in_fMRI_anat_resolution.nii.gz') + \
              ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1, 'Ref_anat_in_fMRI_anat_resolution.nii.gz') + ' -expr "a*b"'
    spco([command], shell=True)
