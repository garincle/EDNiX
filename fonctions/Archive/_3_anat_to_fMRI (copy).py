...
def Refimg_to_meanfMRI(anat_func_same_space, volumes_dir, BASE_SS_coregistr, TfMRI, correction_direction, dir_fMRI_Refth_RS_prepro1, dir_fMRI_Refth_RS_prepro2, 
    dir_fMRI_Refth_RS_prepro3, RS, nb_run, recordings, REF_int, ID, TR, dir_prepro, n_for_ANTS, brainmask_D, brainmask, V_mask, W_mask, G_mask, dilate_mask, list_atlases, labels_dir, overwrite):
    ...
    if anat_func_same_space == True:
        ...
        #spco([command], shell=True)
        command = '3dAllineate' + overwrite + ' -final NN -warp shift_only -1Dmatrix_apply ' + opj(dir_prepro,ID + '_brain_for_Align_Center.1D') + \
        ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.nii.gz') + \
        ' -input  ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image.nii.gz')
        spco([command], shell=True)
        #command = '3dAutobox' + overwrite + ' -input ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.nii.gz') + ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.nii.gz') + ' -noclust  -overwrite'
        #spco(command, shell=True)
    ...
    ############################### ############################### ############################### 
        ...
        command = '@Align_Centers -overwrite -base ' + opj(dir_fMRI_Refth_RS_prepro3,'BASE_SS_fMRI.nii.gz') + ' -dset ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image.nii.gz') + ' -prefix ' + ' Mean_Image_RcT_1.nii.gz'
        spco([command], shell=True)
        os.chdir('/home/cgarin/')

        command = '3dAllineate' + overwrite + ' -cmass -EPI -final NN -float -twobest 5 -fineblur 0 -warp shift_only -base ' + \
        opj(dir_fMRI_Refth_RS_prepro3,'BASE_SS_fMRI.nii.gz') + ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.nii.gz') + \
        ' -source ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT_1.nii.gz') + ' -nmi -1Dmatrix_save ' + \
        opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT_2.1D')
        spco(command, shell=True)

        command = 'cat ' +  opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT_2.1D') + ' ' +  opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT_2.1D') + ' > ' +  opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.1D')
        spco([command], shell=True)
    ...
    for r in range(0, int(nb_run)):
        ...
        if anat_func_same_space == True:
            ...
            spco([command], shell=True)
            #command = '3dZeropad -prefix ' + opj(dir_fMRI_Refth_RS_prepro1,root_RS + '_xdtrf_2ref_RcT.nii.gz') + ' -master ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.nii.gz') + ' ' + opj(dir_fMRI_Refth_RS_prepro1,root_RS + '_xdtrf_2ref_RcT.nii.gz') + ' -overwrite'
            #spco([command], shell=True)
    ...
    # B0 correction
    command = 'DenoiseImage' + ' -d 3 -i ' +  opj(dir_prepro, ID + '_acpc_cropped' + TfMRI + '.nii.gz') + ' -n Gaussian -r 3 ' + \
    ...
    ' -dxyz ' + delta_x + ' ' + delta_y + ' ' + delta_z + ' ' + \
    ' -input  ' + opj(dir_fMRI_Refth_RS_prepro2, TfMRI + '_NU_denoise_maskeDil.nii.gz')
    spco([command], shell=True)
    ...
    ' -source ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT.nii.gz') + ' -nmi -1Dmatrix_save ' + \
    ...
    ## in func space resample to func 
    command = 'antsApplyTransforms -d 3 -e 3 -i ' + opj(volumes_dir,ID + TfMRI + '_brain.nii.gz') + ' -n ' + n_for_ANTS + \
    ' -r ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image_RcT_SS.nii.gz') + \
    ...
    for i in range(0, int(nb_run)):
        ...
        command = '3dcalc' + overwrite + ' -a ' +   opj(dir_fMRI_Refth_RS_prepro1,'maskDilat.nii.gz') + ' -b ' + opj(dir_fMRI_Refth_RS_prepro1,root_RS + '_xdtrf_2ref_RcT.nii.gz') + \
