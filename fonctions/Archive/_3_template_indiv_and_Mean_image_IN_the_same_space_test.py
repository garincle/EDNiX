...
def Refimg_to_meanfMRI(anat_func_same_space, type_norm, Ref_file, volumes_dir, TfMRI, masks_dir, dir_fMRI_Refth_RS_prepro1, dir_fMRI_Refth_RS_prepro2, RS, nb_run, recordings, REF_int, ID, TR, dir_prepro, n_for_ANTS, overwrite):
    ...
    if anat_func_same_space == 'YES':
        ...
        spco([command], shell=True)
        command = '3dAllineate' + overwrite + ' -interp NN -1Dmatrix_apply ' + mvt_shft + \
        ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro2,'maskDilat_pre2.nii.gz') + \
        ' -master ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image.nii.gz') + \
        ...
        opj(dir_prepro,ID + '_mask_ref_resempled.nii.gz') + ' -input ' + opj(dir_fMRI_Refth_RS_prepro2,'maskDilat_pre2.nii.gz')
        spco([command], shell=True)
    ...
    ############################### ############################### ############################### 
        ...
        spco([command], shell=True)
        '''
        command = '3dcalc' + overwrite + ' -a ' + opj(dir_fMRI_Refth_RS_prepro2,'maskDilat_pre.nii.gz') + ' -b ' + opj(dir_prepro, ID + '_brain_for_Align_Center' + TfMRI + '_NU_denoise.nii.gz') + \
        ' -prefix ' +  opj(dir_prepro, ID + '_' + TfMRI + '_mprage_reorient_NU_SS.nii.gz') + ' -expr "a*b"'
        spco([command], shell=True)
    ...
    command = '3dcalc' + overwrite + ' -a ' +  opj(dir_fMRI_Refth_RS_prepro2,'maskDilat_pre2.nii.gz') + ' -b ' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_Image.nii.gz') + \
    ...
    #### co-register all the func to mean fmri? !!!!!! ### img func to Refanat !!
    ...
    ' -m MI[' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_fMRImage_SS.nii.gz') + ',' + opj(dir_fMRI_Refth_RS_prepro2,'anat_in_func_Allin_AFNI.nii.gz') + ',1,32,Regular,0.2]' + \
    ' -t Syn[0.1,3,0] -f 8x4x2x1 -s 3x2x1x0vox -c [1000x500x250x100,1e-6,10]' + \
    ' -m Mattes[' + opj(dir_fMRI_Refth_RS_prepro1,'Mean_fMRImage_SS.nii.gz') + ',' + opj(dir_fMRI_Refth_RS_prepro2,'anat_in_func_Allin_AFNI.nii.gz') + ',1,4,Regular,0.2]'
    spco([command], shell=True)
    ...
    for i in range(0, int(nb_run)):
        ...
        ' -prefix ' + opj(dir_fMRI_Refth_RS_prepro1, root_RS + '_xdtrfw_masked.nii.gz') + ' -expr "a*b" ' + overwrite
        spco([command], shell=True)
