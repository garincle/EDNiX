...
def anats_to_common(sing_afni,diary_file, anat_filenames, write_dir,
    ...
    nl = 'Run anat.anat_to_common_EMB.anats_to_common'
    ...
    """ Create common template from native anat images and achieve
    their registration to it.

    Parameters
    ----------
    anat_filenames : list of str
        Paths to the anat images.
    ...
    ###########################################################################
    # First copy anat files to make sure the originals are never changed
    ...
    #
    # First we loop through anat scans and correct intensities for bias.
    ...
    for (brain_mask_file, unifized_file) in zip(unifized_files,
        ...
        out_calc_mask = calc(in_file_a=unifized_file,
                             ...
                             expr='a*b',
                             outputtype='NIFTI_GZ')
        command = 'singularity run' + s_bind + sing_afni + ' 3dCM ' + out_calc_mask.outputs.out_file + ' -set 0 0 0'
    ...
    for unifized_file, brain_file in zip(unifized_files, brain_files):
        ...
        command = 'singularity run' + s_bind + sing_afni + ' 3drefit ' + out_calc_mask.outputs.out_file + ' -xorigin_raw ' + str(var1) + ' -yorigin_raw ' + var2 + ' -zorigin_raw ' + var3
    ...
    # anat images to each other's brain's (as defined by the brain
    # masker) CoMs.
    ###########################################################################
