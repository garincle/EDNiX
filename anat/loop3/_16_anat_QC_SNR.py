...
def compute_snr_brain_mask(gray_mask_path, anat_data_path, output_path,root_name,diary_file):
    ...
    :param anat_data_path: Path to single-frame anat data NIfTI file
    ...
    if len(non_zero_values) > 0:
        val = np.arange(1,6)*10
        for i in val:
            noise_threshold = np.percentile(non_zero_values, i)
        ...
        nl = "10% of noise is just 0; noise calculation cannot be completed."
        raise ValueError(run_cmd.error(nl, diary_file))
...
def fwhm(anat_file, mask_file, dir,diary_file, sing_afni):
    ...
    try:
        ...
        if out:
            ...
            vals = info[-2].split(' ')[-1]
            return vals  # Return the combined FWHM value
        else:
...
def anat_QC(info, type_norm, ID, volumes_dir,masks_dir,labels_dir,targetsuffix,listTimage,
            sing_afni, diary_file):

    nl = 'Run anat._16_anat_QC_SNR.anat_QC'
    run_cmd.msg(nl, diary_file, 'HEADER')

    output_results = opj(volumes_dir, 'QC_anat')
