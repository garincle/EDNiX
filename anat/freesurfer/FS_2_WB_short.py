...
def WB_prep(cmd_mris_convert,FS_dir,FS_refs,animal,species,Ref_file,datfile,reference,list_atlas,balsa_folder,BALSAname,
    ...
    for h in range(2):
        ...
        for FS_surf, WB_surf, S_type in zip(['white', 'pial', 'mid'],
            ...
            cmd = (sing_wb + 'wb_command -surface-apply-affine ' + opj(dir_native_resol, animal + '.' + Hmin[h] + '.' + WB_surf + '.surf.gii') + ' ' + opj(vol_dir,'c_ras.mat') + ' ' +
    ...
    WB_label.vol2surfWB(species, list_atlas, labels_dir, dir_native_resol, '.native', Hmin,
    ...
    img = WB_ribbon.create(Ref_file, label_FS_all, animal, dir_native_resol, labels_dir, 'white', ['pial', 'midthickness'], ['ribbon', 'infra'], Hmin, RibbonValue,
                 diary_name, sing_wb)

    WB_ribbon.layers(img, animal, dir_native_resol, labels_dir, masks_dir, 'white', ['pial', 'midthickness'], Ref_file, diary_name, sing_wb)

    if not BALSAname == '':
        dir_native_32 = opj(template_dir, reference, 'surfaces', 'fsaverage_LR_32k')
        dir_balsa_resol = opj(template_dir, BALSAname, 'surfaces', 'Native_resol')
        ...
        WB_ribbon.set(Ref_norm, label_FS_all, animal,
        ...
        if species == 'Macaque':
            ...
            WB_vol_atlas.set(opj(balsa_folder, 'surfaces', 'fsaverage_LR_32k',BALSAname + '_Parcellations_v2.32k_fs_LR.dlabel.nii'),
                             animal, BALSAname, parcellation_list, opj(path_label_code, BALSAname + '_label.txt'), dir_native_resol,
                             dir_native_32,
