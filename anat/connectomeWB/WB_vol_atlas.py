...
def get(BALSAname,path_BALSA,dir_path,path_label,diary_name):

    labelnames = opj(path_label,BALSAname + '_label.txt')

    (path_anat, _, _, _, _, volumes_dir, labels_dir, _,) = getpath.anat(dir_path, BALSAname, BALSAname, '', '', 'native')
    ...
    return (BALSAname,volumes_dir,labels_dir,labelnames,dir_native_resol, dir_native_32, dir_balsa_resol, dir_balsa_32, dir_balsa_64,origFile,
            parcellation_list,surfsamp)


def set(origFile,animal, BALSAname, parcellation_list, labelnames, dir_native_resol,dir_native_32, dir_balsa_resol,dir_balsa_32,dir_balsa_64,volumes_dir, labels_dir, FS_dir,Ref_file, surfsamp,Hmin,
    ...
    gifti(opj(dir_orig, '.'.join([BALSAname, Hmin[0], labelsurf_end])),opj(dir_orig, '.'.join([BALSAname, Hmin[1], labelsurf_end])),
...
def cifti(origFile,animal,BALSAname,dir_orig,dir_native_samp,surfsamp,Hmin,CORTEX,diary_name,sing_wb):
    ...
    for s in range(len(Hmin)):
        cmd = (sing_wb + 'wb_command -cifti-separate ' + opj(dir_orig, '.'.join([animal,BALSAname,labelcifti_end])) +
               ' COLUMN -label ' + CORTEX[s] + ' ' + opj(dir_orig, '.'.join([BALSAname,Hmin[s],labelsurf_end])))
        run_cmd.wb(cmd,diary_name)



def gifti(origFileL,origFileR,animal,BALSAname,dir_orig,dir_balsa_resol,dir_native_resol,surfsamp,Hmin,diary_name,sing_wb):
    ...
    if not opj(dir_orig, '.'.join([BALSAname, Hmin[0], labelsurf_end])):
        shutil.copyfile(origFileL, opj(dir_orig, '.'.join([BALSAname, Hmin[0], labelsurf_end])))
    if not opj(dir_orig, '.'.join([BALSAname, Hmin[1], labelsurf_end])):
        shutil.copyfile(origFileR, opj(dir_orig, '.'.join([BALSAname, Hmin[1], labelsurf_end])))

    for s in range(len(Hmin)):
        cmd = (sing_wb + 'wb_command -label-resample ' + opj(dir_orig, '.'.join([BALSAname, Hmin[s], labelsurf_end])) +
               ' ' + opj(dir_orig,        '.'.join([animal, Hmin[s], 'sphere', labelsurf_end])) +
               ' ' + opj(dir_balsa_resol, '.'.join([animal, Hmin[s], 'sphere', 'reg', 'reg_LR.native', 'surf', 'gii'])) +
               ' BARYCENTRIC ' +
               opj(dir_balsa_resol, '.'.join([animal, Hmin[s], BALSAname, 'fs_LR', 'label', 'gii'])) + ' -largest')
        run_cmd.wb(cmd, diary_name)

    cmd = (sing_wb + 'wb_command -cifti-create-label ' + opj(dir_balsa_resol, '.'.join([animal,BALSAname,'dlabel','nii'])) +
...
def FSannot(animal,BALSAname,parcellation_list,dir_native_resol,FS_dir,Hmin,CORTEX,cmd_mris_convert,diary_name,sing_wb,sing_fs):
    ...
    for i in range(len(parcellation_list)):
        ...
        cmd = (sing_wb + 'wb_command -cifti-reorder ' + opj(dir_native_resol, '.'.join([animal,BALSAname,'dlabel','nii'])) + ' ROW ' +
        ...
        for s in range(len(Hmin)):
            ...
            cmd = (sing_fs + cmd_mris_convert + ' --annot ' +
                   ...
                   opj(FS_dir, animal, 'label', Hmin[s] + 'h.' + animal + '_' + BALSAname +  '-' + parcellation_list[i] + '.annot'))
            run_cmd.wb(cmd,diary_name)
...
def volumes(animal,BALSAname,parcellation_list,labelnames,dir_native_resol,volumes_dir,labels_dir,Ref_file,Hmin,CORTEX,diary_name,sing_wb):
    ...
    for s in range(len(Hmin)):
        cmd = (sing_wb + 'wb_command -cifti-separate ' + opj(dir_native_resol, '.'.join([animal,BALSAname,'dlabel','nii'])) +
               ' COLUMN -label ' + CORTEX[s] +
               ' ' + opj(dir_native_resol, '.'.join(['new',Hmin[s],'label','gii'])))
        run_cmd.wb(cmd,diary_name)

        cmd = (sing_wb + 'wb_command -label-to-volume-mapping ' + opj(dir_native_resol, '.'.join(['new',Hmin[s],'label','gii']))
               + ' ' +
    ...
    cmd = (sing_wb + 'wb_command -volume-label-import ' + opj(labels_dir, '_'.join([animal,'seg-' + BALSAname,'dseg.nii.gz'])) +
           ' ' + labelnames +
