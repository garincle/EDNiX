...
def detect_image_type(anat_img, atlas_img,diary_file):
    """Detect whether anat image is T1 or T2 weighted based on tissue contrast."""
    try:
        # Get numpy arrays
        ...
        if ope(anat_img_path) and ope(fmri_img_path):
            try:
                ...
                if image_type_stats['image_type'] == anat_type_stats['image_type']:
                    ...
                    # Create surrogate anat image matching fMRI contrast
                    ...
                    if surrogate_img is not None:
                        ...
                        # Update anat image path to use surrogate for correlation analysis
                        anat_img_path = surrogate_path
                        anat_img = surrogate_img

                        nl = "Created surrogate anat image matching fMRI contrast"
                ...
                nl = f"Coverage: {coverage_stats['pct_anat_uncovered']:.1f}% of anat not covered by fMRI"
                run_cmd.msg(nl, diary_file, 'OKBLUE')
            ...
            nl = "WARNING: Missing anat or fMRI image for coverage analysis"
            run_cmd.msg(nl, diary_file, 'WARNING')
