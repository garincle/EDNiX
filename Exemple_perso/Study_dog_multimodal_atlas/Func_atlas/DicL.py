...
for bids in ['Auburn_bids', 'BIDS_Dora', 'BIDS_k9', 'BIDS_knee']:
    bids_dir = Load_subject_with_BIDS.linux_path(opj('/scratch2/EDNiX/Dog/' + bids))
    allinfo_study_c = load_bids.Load_BIDS_to_pandas(bids_dir, modalities=['func'], suffixes= ['bold'], extensions=['.nii.gz'])

    oversample_map = True
    oversample_dictionary = True
    min_size = 10
    cut_coords = 10
    alpha = 0.0001
    ...
    templatehigh = opj('/home/cgarin/PycharmProjects/EDNiX/Atlases_library/atlas/mammals/carnivorans/Dog/EDNiX/volumes/Dog_space-acpc_desc-SS_T1w.nii.gz')
    TR = None  # 'value du calculate in s', 'Auto', 'None'

    mask_func     = opj('/home/cgarin/PycharmProjects/EDNiX/Atlases_library/atlas/mammals/carnivorans/Dog/EDNiX/volumes/masks/Dog_desc-Cerebral_Gray_mask.nii.gz') #
    redo = True
    ratio_n_voxel = 0.5
