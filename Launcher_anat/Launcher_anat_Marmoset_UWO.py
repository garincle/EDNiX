...
bids_dir = Tools.Load_subject_with_BIDS.linux_path(opj(r"C:\Users\cgarin\Desktop\BIDS_k9"))  # XXX - path not in OLD
FS_dir    = Tools.Load_subject_with_BIDS.linux_path(opj(MAIN_PATH,'FS_Dir_tmp'))
atlas_dir = Tools.Load_subject_with_BIDS.linux_path(opj(r"C:\Users\cgarin\Documents\EDNiX\Atlas_library\Atlases_V2", species))  # XXX - path not in OLD
Lut_dir = Tools.Load_subject_with_BIDS.linux_path(opj(r"C:\Users\cgarin\Documents\EDNiX\Atlas_library\LUT_files"))  # XXX - path not in OLD

########### Subject loader with BIDS##############
layout= BIDSLayout(bids_dir,  validate=False)  # validate=False from OLD
###report
...
Bold = layout.get(return_type='filename', target='subject', suffix='epi', extension='nii.gz')  # XXX - in OLD it was task='rest'
# Convert the layout to a pandas dataframe
df = layout.to_df()
df.head()

#### Create a pandas sheet for the dataset (I like it, it helps to know what you are about to process)
allinfo_study_c = df[(df['suffix'] == 'T2w') & (df['extension'] == '.nii.gz')]  # From OLD (was 'bold' in NEW)
...
type_of_transform = 'SyNCC'  # From OLD (was 'SyN')
aff_metric_ants = 'MI'

####Choose to normalize using T1 or T2
type_norm = 'T2w'  # From OLD (was 'T1w')
otheranat = ''  # From OLD
orientation = 'LPI'  # From OLD (was 'IPL')
BIDStype = 'sub-{ID}_ses-{Session}_{Timage}.nii*'  # From OLD (was 2)

###masking
masking_img = 'T2w'  # From OLD (was 'T1w')
brain_skullstrip_1 ='CustumThreshold_50'  # From OLD (was '3dSkullStrip_dog_macaque')
#precise
brain_skullstrip_2 ='3dSkullStrip_marmoset'  # From OLD (was 'Custum_QWARPT2')
do_fMRImasks = True
...
Align_img_to_template = '@Align_Centers'  # From OLD (was '3dAllineate')
cost3dAllineate = 'lpa'
...
type_of_transform_stdyT = 'SyNCC'  # From OLD (was 'SyN')
Atemplate_to_Stemplate = 'T2w'  # From OLD (was 'T1w')
template_skullstrip = 'Custum_ANTS_Garin'  # From OLD (was 'Manual')

do_surfacewith = 'T2w'  # From OLD (was 'T1w')
...
BASE_SS     = opj(atlas_dir, 'templateT2.nii.gz')  # From OLD (was 'template_SS.nii.gz')
...
### Block2: step 4,5 (study template mask QC itksnap)
