...
from modalities.anat.loop3 import backtonative, _16_anat_QC_SNR, _100_Data_Clean, _200_Data_QC
from modalities.anat import transfo_T2toT1w
from modalities.anat.freesurfer import FS_finalise, FS_flat
...
def run(ID, Session, data_path, max_ses,creat_study_template,study_template_atlas_folder,BASE_SS,BASE_mask,BASE_atlas_folder,
    ...
    if 'itk_3' in Skip_step:
        run_cmd.msg('INFO: skip step itk_3', diary_file, 'OKGREEN')
    else:
        info = backtonative.get(
            ID, data_path, bids_dir, Session, max_ses,
            targetsuffix, type_norm, BASE_SS, BASE_mask,
            BASE_atlas_folder, study_template_atlas_folder,
            creat_study_template, coregistration_longitudinal,
            reference, 'Final', species, diary_file)
        print(info)

        mask_was_updated = _200_Data_QC._itk_check_masks(
            info, brain_mask, volumes_dir, masks_dir,
            ID, type_norm, sing_itk, diary_file)

        if mask_was_updated:
            run_cmd.msg(
                'INFO: backtonative mask updated — running re-registration '
                'cascade: [0] regenerate SS T1w → [1] step 6 (registration) '
                '→ [2] step 8 (backtonative with fresh transforms).',
                diary_file, 'OKGREEN')

            # Resolve sourcename and BASE_SS_coregistr using the same logic
            # as _loop2.run() so _6_stdyTmax gets the correct reference image.
            if not coregistration_longitudinal:
                if creat_study_template:
                    sourcename_rerun        = 'studyTemplate'
                    BASE_SS_coregistr_rerun = study_template_atlas_folder
                else:
                    sourcename_rerun        = 'EDNiX'
                    BASE_SS_coregistr_rerun = BASE_SS
            else:
                if Session != max_ses:
                    data_path_max = opj(bids_dir, 'sub-' + ID,
                                        'ses-' + str(max_ses))
                    _, _, _, _, _, volumes_dir_max, _, _ = \
                        getpath.anat(data_path_max, reference, '',
                                     False, True, 'native')
                    sourcename_rerun        = 'refSession'
                    BASE_SS_coregistr_rerun = opj(
                        volumes_dir_max,
                        ID + '_space-acpc_desc-SS_' + type_norm + '.nii.gz')
                else:
                    if creat_study_template:
                        sourcename_rerun        = 'studyTemplate'
                        BASE_SS_coregistr_rerun = study_template_atlas_folder
            ...
            # [0] Regenerate SS T1w from the corrected backtonative mask.
            # This must happen BEFORE step 6 because _6_stdyTmax registers
            # space-acpc_desc-SS_{type_norm} to the template — that image
            # must be up to date before the registration runs.
            run_cmd.msg(
                'INFO: [0/2] Regenerating space-acpc_desc-SS_' + type_norm
                + ' from backtonative mask.',
                diary_file, 'OKGREEN')
            backtonative.apply(
                ID, volumes_dir, masks_dir, labels_dir, bids_dir,
                info, listTimage, 'space-acpc_desc-SS-step2',
                n_for_ANTS, list_atlas, path_label_code, type_norm,
                diary_file, sing_afni, sing_wb)

            # [1] Redo registration with corrected SS T1w as moving image.
            run_cmd.msg(
                f'INFO: [1/2] Re-running _6_stdyTmax.nativetoTemplate '
                f'(sourcename={sourcename_rerun}).',
                diary_file, 'OKGREEN')
            from modalities.anat.loop2 import _6_stdyTmax
            _6_stdyTmax.nativetoTemplate(
                sourcename_rerun, ID, listTimage, data_path,
                bids_dir, creat_study_template,
                coregistration_longitudinal, reference,
                '_space-acpc_desc-SS_',
                BASE_SS_coregistr_rerun,
                list_transfo, type_norm, sing_wb, diary_file)

            # [2] Rerun backtonative.apply() with the fresh transforms so
            # native-space labels, masks and SS anatomy all reflect the new
            # registration. backtonative.get() is called again to pick up the
            # newly written transform files from step [1].
            run_cmd.msg(
                'INFO: [2/2] Re-running backtonative.apply() with updated '
                'transforms.',
                diary_file, 'OKGREEN')
            info_updated = backtonative.get(
                ID, data_path, bids_dir, Session, max_ses,
                targetsuffix, type_norm, BASE_SS, BASE_mask,
                BASE_atlas_folder, study_template_atlas_folder,
                creat_study_template, coregistration_longitudinal,
            ...
            backtonative.apply(
                ID, volumes_dir, masks_dir, labels_dir, bids_dir,
                info_updated, listTimage, 'space-acpc_desc-SS-step2',
                n_for_ANTS, list_atlas, path_label_code, type_norm,
                diary_file, sing_afni, sing_wb)
    ...
    if 16 in Skip_step:
        ...
        info = backtonative.get(ID, data_path, bids_dir, Session, max_ses, targetsuffix, type_norm, BASE_SS, BASE_mask,
                                ...
                                creat_study_template, coregistration_longitudinal, reference, 'Final', species, diary_file)
        print(info)
        _16_anat_QC_SNR.anat_QC(info, type_norm, ID, volumes_dir, path_anat, masks_dir, labels_dir, targetsuffix, listTimage,
