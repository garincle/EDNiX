...
from modalities.anat.connectomeWB import WB_label, WB_sphere_reg, WB_vol_atlas, WB_ribbon
...
def WB_prep(cmd_mris_convert, FS_dir, FS_refs, dir_path, animal, species, spacing, resamp, datfile, reference,
    ...
    (path_anat, matrices_dir, _, prepro_dir, dir_native, volumes_dir, labels_dir, masks_dir, wb_template_dir,
     ...
     wb_balsa_dir, wb_balsa_vol, wb_balsa_labels, wb_balsa_masks) = getpath.anat(dir_path, reference, BALSAname, '_',
                                                                                 '_', 'template')

    dir_native_resol, dir_native_32, dir_balsa_resol, dir_balsa_32, dir_balsa_64 = getpath.surf(path_anat, reference,
                                                                                                BALSAname)
    if iso == 0:
    ...
    if not BALSAname == '':
        ...
        if not opi(Ref_norm):
            ...
            norm2template.norm(animal, Ref_file, '', wb_balsa_vol, BALSAname, balsa_brainT1, '',
                               ...
                               list_transfo[refnb]["affmetricT"], list_transfo[refnb]["affmetric"],
                               list_transfo[refnb]["interpol"], diary_name, sing_wb, suffix, 1)
    ...
    #  1.0 Copy Files
    if towarp == 1:
        WB_sphere_reg.copyref(FS_refs, dir_balsa_64, dir_balsa_32, wb_balsa_labels, atlasfile, animal, diary_name,
                              sing_wb)

    # 1.1a get the volumes
    # 1.1b get subcortical structures...

    cmd = sing_fs + 'mri_convert ' + bckFS_cmd + opj(FS_dir, animal, 'mri', 'aseg.mgz') + ' ' + opj(labels_dir,
                                                                                                    animal + '_seg-wmparc_dseg.nii.gz')
    run_cmd.run(cmd, diary_name)
    ...
    for h in range(2):
        ...
        for a in range(len(list_atlas[0])):
            if list_atlas[3][a] == 1:
                if list_atlas[2][a] > 1:
                    WB_label.mergelabel(animal, opj(dir_native_resol), 'native', h, diary_name, list_atlas[0][a],
                                        list_atlas[2][a], sing_wb)

    if towarp == 1:
        # warping of the surfaces
        WB_sphere_reg.warp(animal, ['white', 'pial', 'midthickness'], dir_native_resol, dir_balsa_resol, list_atlas,
                           ref_transfo1,
                           ref_transfo2, conv_voxmm, cmd_mris_convert, FS_dir, 'FS', diary_name, sing_wb, sing_fs)
        ...
        WB_sphere_reg.surf_resamp164(animal, dir_native_resol, dir_balsa_resol, dir_balsa_64, conv_voxmm, FS_refs,
                                     list_atlas, diary_name, sing_wb)
        # resampling in low resolution : 32k
        WB_sphere_reg.surf_resamp32(animal, dir_native_resol, dir_balsa_resol, dir_balsa_32, conv_voxmm, FS_refs,
                                    list_atlas, diary_name, sing_wb)
    ...
    if towarp == 1:
        #     for template space
        WB_ribbon.set(dir_path, Ref_norm, label_FS_all, animal, BALSAname, 'template', Hmin, RibbonValue, diary_name,
                      sing_wb)
    ...
    if species == 'Macaque':
        ...
        WB_vol_atlas.set(origFile, animal, BALSAname, parcellation_list, labelfile, dir_native_resol,
                         dir_native_32,
                         dir_balsa_resol, dir_balsa_32, dir_balsa_64, volumes_dir, labels_dir, FS_dir, Ref_file,
                         ssamp, Hmin,
