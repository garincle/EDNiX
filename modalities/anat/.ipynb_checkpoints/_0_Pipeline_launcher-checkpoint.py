...
import anat.loop1._1_correct_orient
import anat.loop1._2_clean_anat
import anat.studytemplate._3_make_template
import anat._4_create_template_brain
import anat._5_brainT_to_stdyT
import anat._5_brainT_to_stdyT_max
import anat._6_stdyT_to_AtlasT
import anat._7_prepar_aseg
import anat._8_do_fMRImasks
import anat._9_nii_to_mgz
import anat._10_FS_1_white
import anat.loop3._12_make_pial
import anat.loop3._13_FS_freeview
import anat.loop3._14_Finalise
import anat.loop3._15_to_WB
import anat.loop3._200_Data_QC
...
def preprocess_anat(BIDStype, deoblique_exeption1, deoblique_exeption2, deoblique, BASE_mask, coregistration_longitudinal, creat_sutdy_template, 
    ...
    for ID, Session, data_path, max_ses in zip(all_ID, all_Session, all_data_path, max_sessionlist):
        ...
        if 1 in Skip_step:
            ...
            anat.loop1._1_correct_orient.correct_orient(BIDStype, listTimage, path_anat, ID, Session, otheranat, type_norm, deoblique_exeption1, deoblique_exeption2, deoblique, orientation, dir_prepro, IgotbothT1T2, overwrite)

        if 2 in Skip_step:
            print('skip step ' + str(2))

        else:
            anat.loop1._2_clean_anat.clean_anat(cost3dAllineate, bids_dir, listTimage, path_anat, ID, Session, otheranat, type_norm, deoblique_exeption1, deoblique_exeption2, deoblique, orientation, dir_prepro, masking_img, do_manual_crop,
    ...
    if creat_sutdy_template==True:
        ...
        if 3 in Skip_step:
            ...
            anat.loop2._3_make_template.make_template(which_on, all_ID_max, max_session, all_data_path_max, all_ID, all_Session, all_data_path, type_norm, study_template_atlas_forlder, template_skullstrip, BASE_SS, BASE_mask, overwrite)
    ...
    for ID, Session, data_path, max_ses in zip(all_ID, all_Session, all_data_path, max_sessionlist):
        ...
        if 4 in Skip_step:
            ...
            anat._4_create_template_brain.create_indiv_template_brain(dir_prepro, ID, Session, listTimage, volumes_dir, masking_img, brain_skullstrip_1, brain_skullstrip_2,
        ...
        if 5 in Skip_step:
            ...
            anat._5_brainT_to_stdyT.brainT_to_T(dir_prepro, ID, Session, listTimage, n_for_ANTS, dir_transfo, type_norm, BASE_SS_coregistr, Ref_file, volumes_dir, transfo_concat_inv, creat_sutdy_template, which_on, all_ID_max, max_session, all_data_path_max, all_ID, all_Session, all_data_path, study_template_atlas_forlder, otheranat, bids_dir, overwrite)
        ...
        if coregistration_longitudinal == True:
            if Session == max_ses:
                ...
                if 5 in Skip_step:
                    ...
                    anat._5_brainT_to_stdyT_max.brainT_to_T_max(dir_prepro, ID, Session, listTimage, n_for_ANTS,
    ...
    if creat_sutdy_template==True:
        ...
        if 6 in Skip_step:
            ...
            anat._6_stdyT_to_AtlasT.stdyT_to_AtlasT(list_atlases, Aseg_ref, Aseg_refLR, BASE_SS, dir_out, n_for_ANTS, study_template_atlas_forlder, Atemplate_to_Stemplate, overwrite)
    ...
    for ID, Session, data_path, max_ses in zip(all_ID, all_Session, all_data_path, max_sessionlist):
        ...
        if 7 in Skip_step:
            ...
            anat._7_prepar_aseg.prepar_aseg(Ref_file, labels_dir, volumes_dir, masks_dir, dir_transfo, BASE_SS_mask, BASE_SS_coregistr, Aseg_refLR, Aseg_ref, type_norm, ID, transfo_concat, dir_prepro, list_atlases, creat_sutdy_template, dir_out, diratlas_orig, check_visualy_each_img, n_for_ANTS, overwrite)
        ...
        if do_fMRImasks == True:
            ...
            if 8 in Skip_step:
                ...
                anat._8_do_fMRImasks.do_fMRImasks(masks_dir, labels_dir, type_norm, overwrite)
        ...
        if 9 in Skip_step:
            ...
            anat._9_nii_to_mgz.nii_to_mgz(ID, Session, FS_dir, Ref_file, labels_dir, volumes_dir, otheranat, IgotbothT1T2, type_norm, overwrite)
        ...
        if 10 in Skip_step:
            ...
            ########################## White Surface construction ##############################
            anat._10_FS_1_white.White_create(FS_dir, animal_folder)
        ...
        if 11 in Skip_step:
            ...
            ########################## White Surface construction ##############################
            # You can go grab a cup of coffe, it can take more than an hour...
            anat._10_FS_1_white.White_more(FS_dir, animal_folder, FS_buckner40)

        if 12 in Skip_step:
            print('skip step ' + str(12))

        else:
            anat._loop3._12_make_pial.make_pial(FS_dir, animal_folder, type_norm, otheranat, Hmin, Ref_file, do_surfacewith, overwrite)
        ...
        if check_visualy_each_img == True:
            if 13 in Skip_step:
                ...
                anat._loop3._13_FS_freeview.FS_Freeview(FS_dir, animal_folder, 'pial', Lut_file)

        if 14 in Skip_step:
            print('skip step ' + str(14))

        else:
            anat._loop3._14_Finalise.FS_finalise(FS_dir, animal_folder, FreeSlabel_ctab_list, list_atlases_2, labels_dir, type_norm, Ref_file)
        if 15 in Skip_step:
            print('skip step ' + str(15))

        else:
            anat._loop3._15_to_WB.WB_prep(FS_dir, dir_native, animal_folder, Ref_file, species, list_atlases_2)

        if 200 in Skip_step:
            print('skip step ' + str(200))

        else:
            anat._loop3._200_Data_QC._itk_check_masks(dir_prepro, masks_dir, ID, type_norm)