...
import inspect
from modalities.anat.loop1 import _itk1_QC, _2_clean_anat
from modalities.anat.loop1 import _1_correct_orient
...
def run(ID, Session, data_path, path_rawanat,BIDStype, listTimage,otheranat, listTimage_orig, type_norm, orientation,IgotbothT1T2, force_myelin_same_space, Align_img_to_template,list_transfo,masking_img,
    ...
    if 'itk_1' in Skip_step:
        run_cmd.msg('INFO: skip step itk_1', diary_file, 'OKGREEN')
    else:
        mask_was_updated = _itk1_QC._itk_check_masks(
            data_path, ID, type_norm, sing_itk, diary_file)

        if mask_was_updated:
            run_cmd.msg(
                'INFO: mask updated by user — rerunning step 2 (_2_clean_anat) '
                'to propagate the correction.',
                diary_file, 'OKGREEN')
            _2_clean_anat.clean_anat(
                Align_img_to_template, bids_dir, listTimage,
                list_transfo, ID, Session,
                type_norm, data_path, masking_img, brain_skullstrip_1,
                BASE_SS_coregistr, BASE_SS_mask, BASE_SS,
                IgotbothT1T2, check_visualy_each_img,
                check_visualy_final_mask, overwrite, anat_ref_path,
                sing_afni, sing_fsl, sing_fs, sing_itk, sing_wb,
