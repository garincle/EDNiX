...
from modalities.anat.loop1 import myelin
...
def correct_orient(BIDStype,
    ...
    nl = 'Run  anat._1_correct_orient.correct_orient'
    ...
    anatname3 = opj(dir_prepro, ID + '_space-raw_desc-Bias_')
    ...
    for Im in range(len(listTimage_orig)):
            ...
            if humanP[0] == '':
                if opi(list_anat[0].replace('.nii.gz','.json')):
                    ...
                    try:
                        ...
                        run_cmd.msg('the subject was scanned with the following position parameter : ' + humanP[Im],
                                    diary_file, 'OKGREEN')
                    except:
                        humanP[Im] = humanP[Im]
        ...
        if len(list_anat)==1:
            ...
            if len(dims) > 3:
                os.makedirs(opj(dir_prepro, 'tmp'), exist_ok=True)
            ...
            ###################################
            filtered_anat = []
            for anat_path in list_anat:
                json_path = opj(opd(anat_path), extract_filename(anat_path) + '.json')
                if not opi(json_path):
                    run_cmd.msg('WARNING: no .json for ' + anat_path, diary_file, 'WARNING')
                    filtered_anat.append(anat_path)  # keep it, can't determine if NORM
                    continue
                with open(json_path) as ff:
                    meta = json.load(ff)
                image_type = meta.get('ImageType', [])
                if 'NORM' in image_type:
                    run_cmd.msg('Removing ' + extract_filename(anat_path) + ' (NORM)',
                                diary_file, 'WARNING')
                else:
                    filtered_anat.append(anat_path)
            if len(filtered_anat)>0:
                list_anat = filtered_anat
            else:
                run_cmd.msg('We only found NORM anat images, lets continue with this...',
                            diary_file, 'WARNING')
    ...
    for Im in range(len(ListimageMyeline)):
        ...
        if not orientation == '':
            ...
            run_cmd.run(cmd, diary_file)
            desc = 'Reorientation to ' + orientation + '.'
        else:
    ...
    for Timage in listTimage:
        ...
        json_object = json.dumps(dictionary, indent=2)
        with open(anatname3 + Timage + '.json', "w") as outfile:
            outfile.write(json_object)
