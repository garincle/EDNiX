...
from modalities.anat.loop1 import acpcalign
...
def clean_anat(Align_img_to_template, bids_dir, listTimage, list_transfo, ID, Session,
    ...
    needs_resample = (
            not os.path.exists(BASE_SS_anat_res) or
            not all(round(abs(a - b), 10) == 0
                    for a, b in zip(img.header.get_zooms()[:3],
                                    nib.load(BASE_SS_anat_res).header.get_zooms()[:3])))
    if needs_resample:
        ...
        run_cmd.msg("OK, same resolution — BASE_SS_anat_res reuse.", diary_file, 'OKGREEN')
    ...
    modalities.anat.skullstrip.Skullstrip_method.Skullstrip_method(brain_skullstrip_1, end_maskname,
    ...
    for Timage in listTimage2:
        ...
        if not Align_img_to_template == 'No':
            ...
            new_img = ants.apply_transforms(fixed=REF, moving=IMG,
                                            transformlist=opj(dir_transfo, 'acpc_0GenericAffine.mat'),
                                            interpolator=list_transfo[refnb2]["interpol"], whichtoinvert=[False])
        ...
        ###     QC               !!                                        ###
        ######################################################################
        os.makedirs(opj(bids_dir, 'QC', 'skullstrip_step1'), exist_ok=True)
        os.makedirs(opj(bids_dir, 'QC', 'align_rigid_to_template'), exist_ok=True)
