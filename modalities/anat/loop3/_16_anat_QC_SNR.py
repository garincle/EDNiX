...
def compute_snr_single_frame(atlas_data, anat_data, labels, diary_file, hemisphere_offset=1000):
    """
    Computes SNR for each region in a single-frame anat image.

    :param atlas_data: 3D array with labeled regions
    :param anat_data: 3D anat image data array
    ...
    for label, region_name in labels.items():
        ...
        val = np.arange(1,6)*10
        for i in val:
            noise = np.std(brain_values[brain_values <= np.percentile(brain_values,
                                                                  i)]) if brain_values.size > 0 else np.nan
            if noise == 0:
                if i !=val.max():
                    nl = 'std of noise values = 0, try with ' + str(i+10) + '% of the lowest values in the img'
                    run_cmd.msg(nl,diary_file,'WARNING')
                else:
                    nl = "more than 50% background is the same value; noise calculation cannot be completed."
...
def compute_average_snr_gray_white(snr_results, gray_regions, white_regions):
    ...
    # Function to compute average SNR across all regions for gray and white matter
    # (no left and right separation)
...
def compute_snr_brain_mask(gray_mask_path, anat_data_path, output_path, root_name, diary_file):
    """
    Computes SNR within a specified gray matter mask for single-frame data.

    SNR = mean(signal in GM mask) / std(lowest percentile of non-zero brain voxels).
    Noise is estimated from the lower tail of the intensity distribution, which
    approximates background noise without requiring an explicit background mask.
    If noise estimation fails at all percentile levels, SNR is returned as NaN
    and a warning is logged — the pipeline continues normally.

    Parameters
    ----------
    gray_mask_path : str
        Path to binary gray matter mask NIfTI file.
    anat_data_path : str
        Path to anatomical NIfTI file (3D or 4D; first volume used if 4D).
    output_path : str
        Directory path for output files (currently unused, reserved for future
        histogram saving).
    root_name : str
        Root name for output files.
    diary_file : str
        Path to diary log file.

    Returns
    -------
    snr : float
        SNR value, or np.nan if signal mask is empty or noise cannot be estimated.
    """
    # ---- Load images --------------------------------------------------------
    try:
        gray_mask = np.rint(nib.load(gray_mask_path).get_fdata()).astype(np.int32)
        anat_data = nib.load(anat_data_path).get_fdata()
    except Exception as exc:
        run_cmd.msg(f'WARNING: compute_snr_brain_mask — could not load images: {exc}',
                    diary_file, 'WARNING')
        return np.nan

    # ---- Handle 4D input (take first volume) --------------------------------
    if anat_data.ndim == 4:
        run_cmd.msg('INFO: 4D image detected — using first volume for SNR.',
                    diary_file, 'OKGREEN')
        anat_data = anat_data[..., 0]

    # ---- Signal: mean within GM mask ----------------------------------------
    mask_signal = anat_data[gray_mask > 0]
    if mask_signal.size == 0:
        run_cmd.msg('WARNING: GM mask contains no voxels — SNR set to NaN.',
                    diary_file, 'WARNING')
        return np.nan

    signal_avg = np.mean(mask_signal)

    # ---- Noise: std of lower tail of non-zero brain voxels ------------------
    non_zero_values = anat_data.flatten()
    non_zero_values = non_zero_values[non_zero_values > 0]

    if non_zero_values.size == 0:
        run_cmd.msg('WARNING: no non-zero voxels found — SNR set to NaN.',
                    diary_file, 'WARNING')
        return np.nan
    ...
    for pct in [10, 20, 30, 40, 50]:
        candidate = np.std(
            non_zero_values[non_zero_values <= np.percentile(non_zero_values, pct)])
        if candidate > 0:
            noise = candidate
            run_cmd.msg(f'INFO: noise estimated at {pct}th percentile '
                        f'(std = {noise:.4f}).',
                        diary_file, 'OKGREEN')
            break
        run_cmd.msg(f'WARNING: noise std = 0 at {pct}th percentile, '
                    f'trying higher.', diary_file, 'WARNING')
    else:
        run_cmd.msg(
            'WARNING: noise estimation failed at all percentile levels '
            '(>50% of non-zero voxels have constant intensity). '
            'SNR set to NaN — QC continues.',
            diary_file, 'WARNING')
        return np.nan

    # ---- SNR ----------------------------------------------------------------
    snr = signal_avg / noise
    run_cmd.msg(f'INFO: SNR = {snr:.2f} '
                f'(signal = {signal_avg:.4f}, noise = {noise:.4f}).',
                diary_file, 'OKGREEN')
    return snr
...
def anat_QC(info, type_norm, ID, volumes_dir,path_anat, masks_dir,labels_dir,targetsuffix,listTimage,
    ...
    nl = 'Run anat._16_anat_QC_SNR.anat_QC'
    run_cmd.msg(nl, diary_file, 'HEADER')

    output_results = opj(path_anat, 'QC_anat')
    if not ope(output_results):
        os.mkdir(output_results)

    img_ref = opj(volumes_dir, ID + '_' + targetsuffix + '_' + type_norm + '.nii.gz')
    brain_mask = opj(masks_dir, ID + '_space-acpc_mask.nii.gz')
    atlas_filename = opj(labels_dir, ID + '_seg-EDNIxCSCLR_dseg.nii.gz')
    if not opi(atlas_filename):
        nl = 'WARNING: no available atlas found, this is a requirement for some of QC analysis'
        run_cmd.msg(nl, diary_file, 'WARNING')

    if ope(img_ref):
        for i in range(len(info)):
            for Timage in listTimage:
                anat_filename       = opj(volumes_dir, ID + '_' + targetsuffix + '_' + Timage + '.nii.gz')
                template_in_anat    = opj(info[i][4], '_'.join([info[i][0], 'space-acpc', 'desc-SS', Timage]) + '.nii.gz')
                ...
                if ope(anat_filename):
                        ...
                        atlas_data = nib.load(atlas_filename).get_fdata()[:,:,:,0] # to get only the first volume
                        atlas_data = np.rint(atlas_data).astype(np.int32)
                        anat_data = nib.load(anat_filename).get_fdata()
                        snr_results, noise = compute_snr_single_frame(atlas_data, anat_data, labels,diary_file)
                        ...
                        try:
                            ...
                            nl = 'FWHM calculation failed: ' + str(e)
                            run_cmd.msg(nl, diary_file, 'WARNING')

                            fwhm_val = None

                        # Compute SNR with brain mask
                        compute_snr_brain_mask_val = compute_snr_brain_mask(brain_mask,anat_filename,
                                                                            output_results, Timage,diary_file)
                        ...
                        if fixed_np.shape != moving_np.shape:
                            run_cmd.msg('WARNING: NMI skipped — shape mismatch '
                                        f'{fixed_np.shape} vs {moving_np.shape}',
                                        diary_file, 'WARNING')
                            nmi = np.nan
                        else:
                        ...
                        create_qc_figure(output_results, Timage, snr_results, img_ref,
                                        template_in_anat, average_snr_results, noise,
                                        anat_filename, info[i][1], fwhm_val, nmi,
                                        compute_snr_brain_mask_val, cnr_val, cortical_contrast)
        ...
        nl = 'WARNING: ' + str(img_ref) + ' not found!!'
        run_cmd.msg(nl, diary_file, 'WARNING')
