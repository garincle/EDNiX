...
from modalities.anat.loop2 import _mask2_Data_QC, _5_create_template_brain, _6_stdyTmax
import shutil
...
def run(ID, Session, data_path, max_ses,type_norm,ref_suffix,coregistration_longitudinal,bids_dir, mask_suffix,
    ...
    if 'itk_2' in Skip_step:
        run_cmd.msg('INFO: skip step itk_2', diary_file, 'OKGREEN')
    else:
        run_cmd.msg('INFO: Checking the output of the second masking step.',
                    diary_file, 'OKGREEN')
        output4mask = opj(masks_dir, ID + '_desc-step2_mask.nii.gz')
        end_maskname = '_'.join([ID, 'final', 'mask', '2.nii.gz'])
        input4msk = opj(volumes_dir, ID + '_space-acpc_desc-SS-step1_'
                        + masking_img + '.nii.gz')
        ...
        mask_updated_2 = _mask2_Data_QC._itk_check_masks(
            output4mask, input4msk, end_maskname, masks_dir, sing_itk, diary_file)

        if mask_updated_2:
            run_cmd.msg(
                'INFO: step-2 mask updated by user — rerunning '
                '_5_create_template_brain to propagate the correction.',
                diary_file, 'OKGREEN')
            _5_create_template_brain.create_indiv_template_brain(
                dir_prepro, list_transfo, ID,
                Session, listTimage, volumes_dir, masking_img,
                brain_skullstrip_2, masks_dir, BASE_SS_coregistr,
                BASE_SS_mask, check_visualy_final_mask,
                bids_dir, MNIBcorrect_indiv, sing_afni, sing_fsl,
                sing_fs, sing_itk, sing_synstrip, Unetpath,
                Skip_step, diary_file, preftool)
