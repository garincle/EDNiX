...
from modalities.anat.freesurfer import smallbrain, preFS
...
def surfFS(volume, ctab, change_hd, diary_name, sing_fs, path_code_label, export_fs, sing_afni, sing_wb):
    ...
    # the labeled volume is supposed to be in animal/native/volumes/label/
...
def surfWB(cmd_mris_convert, FS_dir, animal, label, WB_dir, surftype, h, diary_name, sing_fs, sing_wb):
    ...
    cmd = (sing_fs + cmd_mris_convert + ' --annot ' + opj(FS_dir, animal, 'label',
                                                          Hmin[h] + 'h.' + animal + '_' + label + '.annot') +
    ...
    cmd = (sing_wb + 'wb_command -set-map-names ' + newgii + ' -map 1 ' + '_'.join([animal, Hmin[h], label]))
    run_cmd.wb(cmd, diary_name)
...
def mergelabel(animal, WB_dir, surftype, h, diary_name, label, nb, sing_wb):
    ...
    for z in range(nb):
        newlabel.append(
            '-label ' + opj(WB_dir, '.'.join([animal, Hmin[h], label + str(z + 1), surftype, 'label', 'gii'])))
    print(newlabel)
    to_merge = ' '.join(newlabel)

    cmd = (sing_wb + 'wb_command -label-merge ' +
           opj(WB_dir, '.'.join([animal, Hmin[h], label, surftype, 'label', 'gii'])) +
           ' ' + to_merge)
    run_cmd.wb(cmd, diary_name)

    for z in range(nb):
        os.remove(opj(WB_dir, '.'.join([animal, Hmin[h], label + str(z + 1), surftype, 'label', 'gii'])))


def cifti(animal, label, WB_dir, surftype, roi, spec, diary_name, sing_wb):
    nl = 'Create label surfaces for cifti'
    run_cmd.msg(nl, diary_name, 'OKGREEN')

    newcifti = opj(WB_dir, '.'.join([animal, label, 'dlabel', 'nii']))

    cmd = (sing_wb + 'wb_command -cifti-create-label ' + newcifti +
           ' -left-label ' + opj(WB_dir, '.'.join([animal, 'l', label, surftype, 'label', 'gii'])) +
           ' -roi-left ' + opj(WB_dir, '.'.join([animal, 'l', roi, 'shape', 'gii'])) +
           ' -right-label ' + opj(WB_dir, '.'.join([animal, 'r', label, surftype, 'label', 'gii'])) +
           ' -roi-right ' + opj(WB_dir, '.'.join([animal, 'r', roi, 'shape', 'gii'])))
    run_cmd.wb(cmd, diary_name)

    cmd = (sing_wb + 'wb_command -add-to-spec-file ' + spec + ' INVALID ' + newcifti)
    run_cmd.wb(cmd, diary_name)

def surfvolWB_func(volumefile, animal, label, WB_dir, surftype, h, diary_name, sing_wb):
    nl = 'create label surfaces for WB'
    run_cmd.msg(nl, diary_name, 'OKGREEN')

    newgii = opj(WB_dir, '.'.join([animal, Hmin[h], label, surftype, 'func', 'gii']))
...
def vol2surfWB(balsaswitch,animal, list_atlas, vol_dir, WB_dir, surftype, hemi_type, proj, roi, spec, diary_name, sing_wb):
    nl = 'Create label surfaces for WB from volume'
    run_cmd.msg(nl, diary_name, "OKGREEN")


    if surftype == 'native' and balsaswitch == 0:
        surftype2 = ''
    else:
        surftype2 = surftype

    suffix = '.'.join(filter(None, [surftype2, 'surf']))

    for n in range(len(list_atlas[0])):
        nb = list_atlas[2][n]
        if list_atlas[3][n] == 1:
            print(vol_dir)
            print(list_atlas[0][n])
            filelist = sorted(glob.glob(opj(vol_dir, animal + '*_seg-' + list_atlas[0][n] + '_dseg.nii.gz')))
            print(filelist)
            vol = filelist[0] if filelist else ""  # Gets first file path or empty string
            print(vol)

            for h in hemi_type:
                newgii = opj(WB_dir, '.'.join([animal, h, list_atlas[0][n], surftype, 'label', 'gii']))
                if proj == 'ribbon':
                ...
                for z in range(nb):
                    cmd = (sing_wb + 'wb_command -set-map-names ' + newgii
                           + ' -map ' + str(z + 1) + ' ' + list_atlas[0][n] + '-level-' + str(z + 1))
                    run_cmd.wb(cmd, diary_name)
        ...
        cifti(animal, list_atlas[0][n], WB_dir, surftype, roi, spec, diary_name, sing_wb)
