...
import json
import ants
from modalities.anat import skullstrip
opj = os.path.join
...
from modalities.fMRI._1_fMRI_preTTT_in_fMRIspace import _adaptive_n4_correction
def create_indiv_template_brain(dir_prepro, list_transfo, ID, Session, listTimage, volumes_dir,
    ...
    N4_img         = opj(dir_prepro,  ID + '_space-acpc_desc-Bias_')
    SS_img         = opj(volumes_dir, ID + '_space-acpc_desc-SS-step2_')



    skullstrip.Skullstrip_method.Skullstrip_method(brain_skullstrip_2, end_maskname, input4msk, output4mask,
    ...
    for Timage in listTimage:
        ...
        N4, _, _ = _adaptive_n4_correction(
            input_image=input_template + Timage + '.nii.gz',
            output_path=N4_img + Timage + '.nii.gz',
            mask=output4mask,
            auto_tune=True)
