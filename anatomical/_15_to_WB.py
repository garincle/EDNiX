...
def WB_prep(FS_dir, dir_native, animal_folder, Ref_file, species, list_atlases_2,s_bind,afni_sif,fsl_sif,fs_sif,wb_sif,diary_file):
    ...
    nl = 'Run anatomical._15_to_WB.WB_prep'
    ...
    ##### specific to macaque ###### to change for each species!!!!!! XXXX create a roulette to change XXX skip the step if not surface atlas
    ...
    command = 'singularity run' + s_bind + fs_sif + 'mri_convert -odt float ' + Ref_file + ' ' + opj(wb_native_dir,'dummy.nii.gz')
    nl = spgo(command)
    diary.write(f'\n{nl}')
    print(nl)
    ...
    nl = 'INFO: creation of the spec files: done!'
    ...
    nl = 'Creation of ribbon file: done!'
    print(bcolors.OKGREEN + nl + bcolors.ENDC)
    diary.write(f'\n{nl}')
