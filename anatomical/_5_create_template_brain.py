...
def create_indiv_template_brain(dir_prepro, type_of_transform, ID, aff_metric_ants, Session, listTimage, volumes_dir, masking_img, brain_skullstrip_1, brain_skullstrip_2, masks_dir, type_norm, BASE_SS_coregistr, BASE_SS_mask, otheranat,
    ...
    nl = 'Run anatomical._5_create_template_brain.create_indiv_template_brain'
    ...
    step_skullstrip = 2
    output_for_mask =  anatomical.Skullstrip_method.Skullstrip_method(step_skullstrip, template_skullstrip, study_template_atlas_folder, masking_img, brain_skullstrip_1, brain_skullstrip_2, masks_dir, volumes_dir, dir_prepro, type_norm, BASE_SS_coregistr, BASE_SS_mask,
    ...
    ct = datetime.datetime.now()
    diary.write(f'\n{ct}')
    nl = 'Run anatomical._5_create_template_brain.create_indiv_template_brain (section after skulls stripping)'
    diary.write(f'\n{nl}')
    ...
    for Timage in listTimage:
        ...
        diary.write(f'\n{nl}')
        print(nl)
        dictionary = {"Sources": [output_for_mask,
                                  opj(volumes_dir, ID + '_' + Timage + '_template.nii.gz')],
                      "Description": 'Resample.', }
        json_object = json.dumps(dictionary, indent=2)
        with open(opj(masks_dir, ID + '_mask_rsp_' + Timage + '.json'), "w") as outfile:
        ...
        diary.write(f'\n{nl}')
        print(nl)
        dictionary = {"Sources": [opj(volumes_dir, ID + '_' + Timage + '_template.nii.gz'),
                                  opj(masks_dir, ID + '_mask_rsp_' + Timage + '.nii.gz')],
                      "Description": 'Skull strip.', }
        json_object = json.dumps(dictionary, indent=2)
        with open(opj(volumes_dir,ID + Timage + '_brain_step_1.json'), "w") as outfile:
            outfile.write(json_object)
    ...
    try:
        ...
        dictionary = {"Sources": [opj(volumes_dir,ID + type_norm + '_brain_step_1.nii.gz'),
                                  output_for_mask],
                      "Description": 'Non uniformity bias field correction (N4 from ANTSpy).', }
        json_object = json.dumps(dictionary, indent=2)
        with open(opj(dir_prepro,ID + '_' + type_norm + '_brain_N4.json'), "w") as outfile:
            outfile.write(json_object)

        try:
            # Load the NIfTI image
            img = nib.load(opj(dir_prepro,ID + '_' + type_norm + '_brain_N4.nii.gz'))
            # Get the image data
            data = img.get_fdata()
