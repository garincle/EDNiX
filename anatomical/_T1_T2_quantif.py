...
def QuantifT1T2(all_ID, all_Session, all_data_path, IgotbothT1T2, list_atlases, otheranat, type_norm, max_sessionlist, overwrite_option):
    ...
    for ID, Session, data_path, max_ses in zip(all_ID, all_Session, all_data_path, max_sessionlist):
        ...
        for atlas in list_atlases:
            ...
            command = '3dZeropad -I 200 -S 200 -A 200 -P 200 -L 200 -R 200 -S 200 -prefix ' + opj(wb_T1T2_dir, opb(atlas)) + ' ' + opj(labels_dir, type_norm + opb(atlas)) + ' -overwrite'
            spco([command], shell=True)

            command = '3dAllineate' + overwrite + ' -overwrite -NN -1Dmatrix_apply ' + mvt_shft + \
            ' -prefix ' + opj(wb_T1T2_dir, opb(atlas)) + \
            ' -input  ' + opj(wb_T1T2_dir, opb(atlas))
            spco([command], shell=True)

            spco(['3dresample', '-master', opj(dir_prepro, ID + '_mprage_reorient' + type_norm + '.nii.gz'), '-prefix',
                  opj(wb_T1T2_dir, opb(atlas)), '-input',
                  opj(wb_T1T2_dir, opb(atlas)), '-overwrite'])
        '''
        ##### apply the recenter fmri
        command = '3dZeropad -I 200 -S 200 -A 200 -P 200 -L 200 -R 200 -S 200 -prefix ' + opj(wb_T1T2_dir, opb(brainmask)) + ' ' + brainmask + ' -overwrite'
        spco([command], shell=True)
        ...
        command = '3dcalc' + overwrite + ' -a ' + opj(dir_prepro, ID + '_mprage_reorient' + type_norm + '.nii.gz') + ' -b ' +  opj(wb_T1T2_dir, 'brain_mask_in_anat_DC.nii.gz') + \
        ...
        spco(['3dcalc', '-a', opj(wb_T1T2_dir, ID + '_' + type_norm + '_brain_RSPL.nii.gz'), '-b',
        ...
        command = '3dcalc -overwrite -a ' + opj(wb_T1T2_dir, ID + '_' + type_norm + '_brain_RSPL.nii.gz') + \
