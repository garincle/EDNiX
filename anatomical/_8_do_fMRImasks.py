...
def do_fMRImasks(masks_dir, labels_dir, type_norm, overwrite):
    # for RS analysis
    ...
    command = '3dcalc' + overwrite + ' -a ' + opj(labels_dir, type_norm + 'aseg.nii.gz') + ' -expr "amongst(a' + Ventri_mask + ')" -prefix ' + opj(masks_dir, type_norm + 'Vmask.nii.gz')
    ...
    command = '3dcalc' + overwrite + ' -a ' + opj(labels_dir, type_norm + 'aseg.nii.gz') + ' -expr "amongst(a' + White_mask + ')" -prefix ' + opj(masks_dir, type_norm + 'Wmask.nii.gz')
    spco([command], shell=True)
