...
def extractSulc(Pd_allinfo_study, regressor_list, all_ID, all_Session, all_data_path, segmentation_name_list, segmentation_ID_list, atlas_names_Seg_list, atlas_ref_list, bids_dir, overwrite_option):
    ...
    for segmentation, IDSEG, atlas_ref, atlas_names_Seg in zip(atlas_names_Seg_list, segmentation_ID_list, atlas_ref_list, segmentation_name_list):
        ...
        for ID, Session, data_path in zip(all_ID, all_Session, all_data_path):
            ...
            if ope(Folder_ROIs + '/sulcus.xlsx') == False or ope(Folder_ROIs + '/sulcus.xlsx') == True and overwrite_option == True:
                print(ID)
                print(Session)
                print(data_path)
                for index, row in atlas_names_Seg.iterrows():
                    ...
                    for i in range(0, 2):
                        ...
                        DATA = spco('wb_command -metric-vertex-sum ' + opj(dir_native_resol, animal_folder  + '.' + h[i] + '.sulc.shape.gii') + \
                        ...
                        print('sulcus of ' + str(DATA/Vertex))

                        atlas_regions.append(str(key) + '_' + str(h[i]))
                        indiv_pd.to_excel(Folder_ROIs + '/sulcus.xlsx')

            else:
                indiv_pd = pd.read_excel(Folder_ROIs + '/sulcus.xlsx')
        ...
        for regressor in regressor_list:
            ...
            out_results_V = opj(out_results, 'sulcus')
