...
import analyses.Group_fMRI._Group_anal_3dTtest
...
bids_dir = '/srv/projects/easymribrain/scratch/'
...
import pandas as pd
selected_atlases = ['atlaslvl3_LR.nii.gz', 'registered_bodiesfaces_mask.nii', 'registered_bodiesfaces_LR.nii']  # Your selected atlases for SBA
panda_files = [pd.DataFrame({'region': ['auditory', 'auditory'], 'label': [64, 1064]}), pd.DataFrame({'region': ['bodies-faces'], 'label': [1]}),
    pd.DataFrame({'region': ['R_bodies-faces', 'L_bodies-faces'], 'label': [2, 1]})]

endfmri = '*_task-rest_*.nii.gz' # string

all_ID, all_Session, all_data_path, all_ID_max, all_Session_max, all_data_path_max, mean_imgs, images_dir =  Tools.Load_BIDS_data_for_analysis.reverse_load_data_bids(bids_dir, 'Specific', file_pattern="residual_in_template.nii.gz")
...
#Statistics._Group_anal__func_DicLearn.dicstat(oversample_map, mask_func, cut_coords, alpha_dic, component_list, oversample_dictionary,
