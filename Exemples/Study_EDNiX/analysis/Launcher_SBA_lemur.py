...
# ?? BIDS dataset ??????????????????????????????????????????????????????????????
BIDS_DIR         = os.path.join('/scratch2/EDNiX/Mouse_lemur/BIDS_Garin')
...
TEMPLATE_LOW     = '/scratch2/EDNiX/Mouse_lemur/BIDS_Garin/sub-147BCBB/ses-01/func/templates/EDNiX/preprocessing/BASE_SS_fMRI.nii.gz'
