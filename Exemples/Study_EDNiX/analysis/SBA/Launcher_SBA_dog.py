...
SPECIES_NAME     = 'Dog'           # used in surface filenames
SPECIES_FRAGMENT = 'atlas/mammals/carnivorans/Dog'

# ?? BIDS dataset ??????????????????????????????????????????????????????????????
BIDS_DIR         = os.path.join('/scratch2/EDNiX/Dog/BIDS_k9')
...
TEMPLATE_LOW     = '/scratch2/EDNiX/Dog/BIDS_k9/sub-01/ses-1/func/templates/EDNiX/preprocessing/BASE_SS_fMRI.nii.gz'
