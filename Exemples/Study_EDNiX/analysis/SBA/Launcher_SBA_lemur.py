...
from Statistics.Group_fMRI import _Group_anal_3dLMEr_SBA
from Statistics.Group_fMRI._Group_anal_3dTtest import _3dttest_EDNiX
from Statistics.Group_fMRI import _Group_anal_3dLMEr_Mirror
...
SPECIES_NAME     = 'Mouselemur'           # used in surface filenames
SPECIES_FRAGMENT = 'atlas/mammals/primates/Mouselemur'

# ?? BIDS dataset ??????????????????????????????????????????????????????????????
BIDS_DIR         = os.path.join('/scratch2/EDNiX/Mouselemur/BIDS_Garin')
...
TEMPLATE_LOW     = BIDS_DIR + '/sub-147BCBB/ses-01/func/templates/EDNiX/preprocessing/BASE_SS_fMRI.nii.gz'

# ?? Singularity / wb_command ??????????????????????????????????????????????????
SING_WB = (
    'vglrun singularity exec '
    f'--bind {ATLAS_LIB}/,/scratch2/,/scratch/ '
    f'{MAIN_PATH}/Singularity_library/Singularity/workbench_2.1.0.sif'
)
...
allinfo_study_c = load_bids.Load_BIDS_to_pandas(
    BIDS_DIR, modalities=['anat'], suffixes=['T2w'], extensions=['.nii.gz']
)
...
_3dttest_EDNiX(**SHARED)

_Group_anal_3dLMEr_SBA._3dLMEr_EDNiX(
    **SHARED,
    model    = '"Sess*(1|Subj)"',
    gltCode  = "-gltCode groupeffect 'Sess : 0.5*Sess_01 +0.5*Sess_02'",
    stat_img = ['groupeffect'],
)
_Group_anal_3dLMEr_Mirror._3dLMEr_EDNiX(
    **SHARED,
...
niftotoWBsurface.render_stat_maps(
    ...
    stat_suffixes       = ['groupeffect_zstat', 'groupeffect_coef'],
    regions_of_interest = REGIONS_OF_INTEREST,
    surface_cfg         = SURFACE_CFG,
)
